...
// Receive messages from popup.js and forward it to the injected code (inpage.ts)
...
// Receive messages from injected inpage.ts code
window.addEventListener("message", async (e) => {
  ...
  switch (e.data.type) {
    case "i_switchEthereumChain": {
      ...
      store.chainName = chainName!;
      // send message to switchEthereumChain with RPC, in inpage.ts
      window.postMessage(
        {
